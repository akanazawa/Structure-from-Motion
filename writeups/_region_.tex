...
\vec x &= M\vec X + \vec t
\end{align*}
Where $M = (\begin{smallmatrix}
  1& 0&0 \\0 & 1 & 0
)\end{smallmatrix}R \in \mathbf{R}^{2\times 3}$ is referred to as the projection matrix, and $R$
...
Let $L = QQ^T$. Recall that $\hat M$ is made of orthonormal unit vectors where the first
$F$ entries of $\hat M$, $\hat i_f\in 
\message{ !name(SfM.tex) !offset(14) }
